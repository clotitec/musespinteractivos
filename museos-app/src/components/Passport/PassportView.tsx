    ...
    <div className="space-y-8">
      ...
      <div className="border border-neutral-200 p-8">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-16 h-16 bg-neutral-100 flex items-center justify-center text-3xl">
            <Trophy className="text-neutral-900" />
          </div>
          <div>
            <h2 className="text-2xl font-display font-bold text-neutral-900">Pasaporte Cultural</h2>
            <p className="text-sm text-neutral-400">
              Nivel: <span className="text-neutral-900 font-semibold">{level}</span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {achievements.map(({ icon, label, value, total }) => (
            <div key={label} className="bg-neutral-50 p-4 text-center">
              <span className="text-2xl">{icon}</span>
              <p className="text-xl font-bold text-neutral-900 mt-1">
                {value}{total !== null ? <span className="text-xs text-neutral-400">/{total}</span> : null}
              </p>
      ...
      {visitedMuseums.length > 0 && (
        <div>
          <h3 className="flex items-center gap-2 text-sm font-semibold text-neutral-900 uppercase tracking-wider mb-4">
            <MapPin size={16} className="text-neutral-900" /> Visitados ({visitedMuseums.length})
          </h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      ...
      {visited.length === 0 && favorites.length === 0 && (
        <div className="text-center py-20 text-neutral-400">
          <Star size={48} className="mx-auto mb-4 text-neutral-200" />
