...
export default function MuseumCard({ museum, isVisited, isFavorite, onToggleFavorite, onToggleVisited }: MuseumCardProps) {
  return (
    <div className="group bg-gray-900/50 border border-gray-800 rounded-xl overflow-hidden hover:border-purple-500/30 transition-all hover:shadow-lg hover:shadow-purple-500/5">
      ...
      <div className="h-32 bg-gradient-to-br from-purple-600/20 to-amber-600/20 relative flex items-end p-4">
        ...
        {museum.es_gratuito && (
          <span className="absolute top-3 left-3 bg-green-500/20 text-green-400 text-[10px] font-medium px-2 py-0.5 rounded-full border border-green-500/30">
            Gratuito
          </span>
        )}
        <div className="flex gap-1 absolute bottom-3 right-3">
          {onToggleFavorite && (
            <button
              ...
              className={`p-1.5 rounded-lg transition-all ${isFavorite ? 'bg-red-500/20 text-red-400' : 'bg-gray-800/80 text-gray-500 hover:text-red-400'}`}
            >
          ...
          {onToggleVisited && (
            <button
              ...
              className={`p-1.5 rounded-lg transition-all ${isVisited ? 'bg-green-500/20 text-green-400' : 'bg-gray-800/80 text-gray-500 hover:text-green-400'}`}
            >
              <Check size={14} />
            </button>
          )}
      ...
      <div className="p-4">
        <Link href={`/museo/${museum.slug}`}>
          <h3 className="font-semibold text-sm text-gray-200 line-clamp-2 group-hover:text-purple-300 transition-colors mb-2">
            {museum.nombre}
          </h3>
        </Link>
