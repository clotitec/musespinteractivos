...
export default function MuseoDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!museum) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-20 text-center">
        ...
        <h2 className="text-2xl font-bold text-gray-200 mb-2">Museo no encontrado</h2>
        <Link href="/" className="text-purple-400 hover:text-purple-300">Volver al mapa</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
      {/* Back link */}
      <Link href="/" className="inline-flex items-center gap-2 text-sm text-gray-400 hover:text-gray-200 transition-colors">
        <ArrowLeft size={16} /> Volver
      </Link>

      {/* Hero */}
      <div className="bg-gradient-to-br from-purple-600/20 to-amber-600/20 rounded-2xl p-8 border border-gray-800">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            <h1 className="text-3xl font-bold text-gray-100 mb-2">{museum.nombre}</h1>
            {museum.municipio && (
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`p-3 rounded-xl transition-all ${isFav ? 'bg-red-500/20 text-red-400' : 'bg-gray-800 text-gray-500 hover:text-red-400'}`}
            >
              <Heart size={20} fill={isFav ? 'currentColor' : 'none'} />
            </button>
            <button
              onClick={() => passport.toggleVisited(museum.id)}
              className={`p-3 rounded-xl transition-all ${isVisited ? 'bg-green-500/20 text-green-400' : 'bg-gray-800 text-gray-500 hover:text-green-400'}`}
            >
              <Check size={20} />
            </button>
          </div>
      ...
      {museum.descripcion && (
        <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
          <h3 className="text-sm font-semibold text-gray-300 mb-3">Descripci&oacute;n</h3>
          <p className="text-gray-400 text-sm leading-relaxed">{museum.descripcion}</p>
        </div>
      )}

      {/* Info grid */}
      <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
        <h3 className="text-sm font-semibold text-gray-300 mb-4">Informaci&oacute;n</h3>
      ...
      {museum.web && (
        <a
          ...
          className="flex items-center justify-center gap-2 bg-purple-600/20 text-purple-400 border border-purple-500/20 rounded-xl p-4 text-sm hover:bg-purple-600/30 transition-colors"
        >
      ...
      {museum.lat && museum.lng && (
        <div className="bg-gray-900/50 border border-gray-800 rounded-xl p-6">
          <h3 className="text-sm font-semibold text-gray-300 mb-3">Ubicaci&oacute;n</h3>
          <a
